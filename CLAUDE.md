...
### ZUI Framework
- [x] **FIXED — Animation/crossfade was broken.** Root cause: Unity 2021.2+'s 7-parameter `GUI.DrawTexture` overload uses its `Color` parameter as a direct tint, bypassing `GUI.color`. `DrawVisualLerped` set `GUI.color` alpha for crossfade, but `ZUIGradient.DrawRect` passed explicit colors to the 7-param overload, so the alpha was ignored — both states rendered at full opacity, making "to" cover "from" instantly. Fix: all drawing paths now multiply their explicit color by `GUI.color` before passing to the 7-param overload. Affected: `ZUIGradient.DrawRect` (solid + gradient), `DrawGlowEffect`, `DrawShadowEffect`, `DrawPatternEffect`.
...
- [x] **FIXED — Style sheet isolation broken.** Two root causes: (1) `ZUIButtonDef`/`ZUIBoxDef` resolution methods (`ResolveBoxStyle`, `GetLabelStyle` global padding, etc.) read `ZUI.ActiveSheet` instead of their owning sheet, so cached GUIStyles baked values from whichever sheet happened to be active. Fix: added `ownerSheet` field to defs, wired by `ZUIStyleSheetAsset.WireOwnerSheet()` on enable and `BumpVersion()`. All resolution now uses `ownerSheet`. (2) Chrome buttons (copy/paste) in style editor section headers drew inside `UseSheet(_sheet)` scope, picking up edited sheet styles. Fix: `ChromeButton()` helper scopes to `EditorSheet`. Also added `BumpVersion()` + version tracking on `ZUIStyleSheetAsset` so `ZUIWindow` auto-invalidates caches when any sheet is modified externally.
...
- [ ] **Real-time DSP preview via `OnAudioFilterRead`** — Currently ALL Klip effects (EQ, gain, compression, trim) are offline-rendered. `KlipHandler.m_isRealtime = false` is hardcoded (line 18), making the real-time pitch/volume code dead. Plan: add `ZoundPreviewFilter : MonoBehaviour` with `OnAudioFilterRead` for real-time preview of EQ, gain, and compression during editing. Refactor biquad filters into shared class usable both offline (batch) and real-time (per-buffer). Pitch envelope stays offline-only — `OnAudioFilterRead` can't change playback rate, and updating `AudioSource.pitch` from `Update()` is too coarse (~10ms steps = audible stepping). Trim can be real-time via `AudioSource.time`. Click "Render" to bake final clip as before.

### Zounds UI
- [ ] **Browser row rework** — rows are currently inconsistent height: some 1 row, others 2, depending on per-zound tag overflow (`BrowserTab.cs:1234`). Plan: remove inline tags from rows entirely. Show a subtle "Tags" indicator if zound has tags. When tag filters are active, show only the matching filtered tags (small known set, always fits one row). This eliminates per-zound height differences.
- [ ] Unify Klip/Zequence row controls — use ZUI.Form-based layout
- [ ] Replace raw `GUILayout.Space` in browser views with ZUI spacing
- [ ] File/Missing entries — minimal (name button only)
